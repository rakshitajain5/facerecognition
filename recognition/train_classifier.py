...
import recognition.lfw_input
from recognition.lfw_input import filter_dataset, split_dataset, get_dataset

logger = logging.getLogger(__name__)


def main(input_directory, model_path, classifier_output_path, batch_size, num_threads, num_epochs,
         min_images_per_labels, is_train=True):
    """
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        #
        # images, labels, class_names = _load_images_and_labels(train_set, image_size=180, batch_size=batch_size,
        #                                                           num_threads=num_threads, num_epochs=num_epochs,
        #                                                           random_flip=True, random_brightness=True,
        #                                                           random_contrast=True)
...
def _get_test_and_train_set(input_dir, min_num_images_per_label):
    """
    Load train dataset. Classes with < :param min_num_images_per_label will be filtered out.
    :param input_dir:
...
def _load_images_and_labels(dataset, image_size, batch_size, num_threads, num_epochs, random_flip=False,
    ...
    image_paths, labels = recognition.lfw_input.get_image_paths_and_labels(dataset)
    images, labels = recognition.lfw_input.read_data(image_paths, labels, image_size, batch_size, num_epochs, num_threads,
...
if __name__ == '__main__':
    ...
    main(input_directory=args.input_dir, model_path=args.model_path, classifier_output_path=args.classifier_path,
         ...
         min_images_per_labels=args.min_images_per_class, is_train=args.is_train)
